...
1. Git clone ````https://github.com/PROCuuuba/Task1````
...
Запустив приложение вам будen доступны 3 кнопки:
* ````Сделать фото````, по нажатию данной кнопки откроется приложение камеры, после чего нужно будет сделать снимок. Далее вылезет всплывающее окно с выбором места отправки данного снимка.
* ````Совершить вызов````, по нажатию этой кнопки откроется окно с одной кнопкой ````Совершить вызов````, нажав её вас перенесёт на ваше приложение для вызовов где будет автоматически введён номер, который был записан в коде программы.
* ````Открыть веб-сраницу````, по нажатию данной кнопки будет открыт браузер с сайтом Воронежского Государстенного Технического университета ````https://cchgeu.ru/````.
